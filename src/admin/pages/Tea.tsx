...
export default function Tea() {
  ...
  const handleSave = async () => {
    ...
    try {
      await fetch(
        `https://34df94e0332d52dc.mokky.dev/tea/${editedProduct.id}`,
        {
          method: "PATCH",
  ...
  const handleDelete = async (id: number) => {
    try {
      await fetch(`https://34df94e0332d52dc.mokky.dev/tea/${id}`, {
        method: "DELETE",
      });
  ...
  const handleAddProduct = async () => {
    ...
    try {
      const res = await fetch("https://34df94e0332d52dc.mokky.dev/tea", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <section className="cofe">
      ...
      >
        <h1 className="cofe-title">Choy</h1>
        <Button
          type="primary"
          icon={<PlusOutlined />}
